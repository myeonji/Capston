...
function Itemlist() {
  ...
  return (
    <div className='Body'>
      ...
      <hr></hr>
      <div className='grid'>
        {shoes.map((i, a) => (<Card shoes={shoes[a]} i={a} key={a}></Card>))}
      </div>
      <hr />
      <h4>Upload New Product</h4>
      <ProductForm /> {/* Add ProductForm component here */}
    </div>
  );
}
  ...
  return (
    <div className="Modal">
      <Row>
        ...
        <Col><div>
          <h5>Sort By</h5>
          <button className='sortBtn' onClick={() => { setView(!view) }}>{sort}</button>
          <ul>
